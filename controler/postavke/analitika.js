   ...
   function(error, results){
           ...
           function(error, results){
               ...
               (
                   ...
                   left join product_items on products.id = product_items.product_id 
                   where 
                       sale_items.canceled_by is null and
                   ...
                   select 
                       ...
                       0 as ulazi,
                       0 prodaja,
                       sum(cast(sale_items.amount * product_items.amount as DECIMAL(12,4))) as otpisi,
                       0 as poslije
                   from 
                   sale_items 
                   left join orders on sale_items.order_id = orders.id
                   left join products on products.id = sale_items.product_id
                   left join product_items on products.id = product_items.product_id 
                   where 
                       orders.table_id = 46
                       /*and sale_items.canceled_by is null*/
                       and sale_items.date > ? and sale_items.date < ?
                   group by
                       product_items.article_id
                       )
               ) as xyz
               ...
               group by 
                   ...
                   `,[req.query.id2, req.query.id1, req.query.id2, req.query.datum2, req.query.datum1, req.query.datum2, req.query.datum1, req.query.datum2, req.query.datum1], 
                   function(error, results){
                           if(error) {
                               return res.status(500).json({
                                   title: 'An error has occured',
